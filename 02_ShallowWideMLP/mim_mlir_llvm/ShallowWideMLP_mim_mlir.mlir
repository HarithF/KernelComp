module {
  func.func public @ShallowWideMLP(%l_self_modules_network_modules_0_parameters_weight_: tensor<32768x16384xf32>, %l_self_modules_network_modules_0_parameters_bias_: tensor<32768xf32>, %l_x_: tensor<128x16384xf32>, %l_self_modules_network_modules_2_parameters_weight_: tensor<32768x32768xf32>, %l_self_modules_network_modules_2_parameters_bias_: tensor<32768xf32>, %l_self_modules_network_modules_4_parameters_weight_: tensor<16384x32768xf32>, %l_self_modules_network_modules_4_parameters_bias_: tensor<16384xf32>) -> tensor<128x16384xf32> {
    ...
    %v12.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4) -> (d0, d2, (d4 + (d0 * 2 + d2) * 32768) floordiv 4 mod 8192, d4 mod 4)>, affine_map<(d0, d1, d2, d3, d4) -> (d1 * 4 + d3, d4)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d2, d1, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel", "reduction"]}
    ...
    %v23.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3, d4) -> (d0, d2, (d4 + (d0 * 2 + d2) * 32768) floordiv 4 mod 8192, d4 mod 4)>, affine_map<(d0, d1, d2, d3, d4) -> (d1 * 4 + d3, d4)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d2, d1, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel", "reduction"]}
    ...
    %input_5.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0 floordiv 2, d0 mod 2, (d1 + d0 * 16384) floordiv 4 mod 4096, d1 mod 4)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]}
