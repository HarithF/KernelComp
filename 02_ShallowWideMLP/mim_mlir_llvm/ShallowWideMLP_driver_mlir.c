...
/* Auto-generated driver for `ShallowWideMLP` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./02_ShallowWideMLP/mim_mlir_llvm/ShallowWideMLP_mim_mlir.mlir)
...
extern memref2d ShallowWideMLP(
    float *w1_a,
    float *w1_al,
    int64_t w1_off,
    int64_t w1_s0,
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "ShallowWideMLP_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  write_timing_report(json_report_path, "ShallowWideMLP", "mim_mlir_llvm", run_id,
