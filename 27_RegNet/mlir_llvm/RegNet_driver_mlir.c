...
/* Auto-generated driver for `RegNet` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./27_RegNet/mlir_llvm/RegNet_mlir.mlir)
 ...
 * in weights.bin is its byte offset from RegNet_params.json,
 ...
 * b2: shape [64] (bias) <- feature_extractor.0.1.weight @ byte 7680
 * b3: shape [64] (bias) <- feature_extractor.0.1.bias @ byte 7936
 * w2: shape [64, 64, 3, 3] (weight) <- feature_extractor.0.3.weight @ byte 8192
 * b4: shape [64] (bias) <- feature_extractor.0.3.bias @ byte 155648
 * b5: shape [64] (bias) <- feature_extractor.0.4.weight @ byte 156416
 * b6: shape [64] (bias) <- feature_extractor.0.4.bias @ byte 156672
 * w3: shape [128, 64, 3, 3] (weight) <- feature_extractor.1.0.weight @ byte 156928
 * b7: shape [128] (bias) <- feature_extractor.1.0.bias @ byte 451840
 * b8: shape [128] (bias) <- feature_extractor.1.1.weight @ byte 453376
 * b9: shape [128] (bias) <- feature_extractor.1.1.bias @ byte 453888
 * w4: shape [128, 128, 3, 3] (weight) <- feature_extractor.1.3.weight @ byte 454400
 * b10: shape [128] (bias) <- feature_extractor.1.3.bias @ byte 1044224
 * b11: shape [128] (bias) <- feature_extractor.1.4.weight @ byte 1045760
 * b12: shape [128] (bias) <- feature_extractor.1.4.bias @ byte 1046272
 * w5: shape [256, 128, 3, 3] (weight) <- feature_extractor.2.0.weight @ byte 1046784
 * b13: shape [256] (bias) <- feature_extractor.2.0.bias @ byte 2226432
 * b14: shape [256] (bias) <- feature_extractor.2.1.weight @ byte 2229504
 * b15: shape [256] (bias) <- feature_extractor.2.1.bias @ byte 2230528
 * w6: shape [256, 256, 3, 3] (weight) <- feature_extractor.2.3.weight @ byte 2231552
 * b16: shape [256] (bias) <- feature_extractor.2.3.bias @ byte 4590848
 * b17: shape [256] (bias) <- feature_extractor.2.4.weight @ byte 4593920
 * b18: shape [256] (bias) <- feature_extractor.2.4.bias @ byte 4594944
 * w7: shape [10, 256] (weight) <- fc.weight @ byte 4595968
 * b19: shape [10] (bias) <- fc.bias @ byte 4606208
 * b20: shape [64] (bias) <- feature_extractor.0.1.running_mean @ byte 7168
 * b21: shape [64] (bias) <- feature_extractor.0.1.running_var @ byte 7424
 * b22: shape [64] (bias) <- feature_extractor.0.4.running_mean @ byte 155904
 * b23: shape [64] (bias) <- feature_extractor.0.4.running_var @ byte 156160
 * b24: shape [128] (bias) <- feature_extractor.1.1.running_mean @ byte 452352
 * b25: shape [128] (bias) <- feature_extractor.1.1.running_var @ byte 452864
 * b26: shape [128] (bias) <- feature_extractor.1.4.running_mean @ byte 1044736
 * b27: shape [128] (bias) <- feature_extractor.1.4.running_var @ byte 1045248
 * b28: shape [256] (bias) <- feature_extractor.2.1.running_mean @ byte 2227456
 * b29: shape [256] (bias) <- feature_extractor.2.1.running_var @ byte 2228480
 * b30: shape [256] (bias) <- feature_extractor.2.4.running_mean @ byte 4591872
 * b31: shape [256] (bias) <- feature_extractor.2.4.running_var @ byte 4592896
 * output: shape [8, 10]
 *
 * Usage: RegNet_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
static const size_t b7_count = 128;
static const size_t b8_count = 128;
...
static const size_t b19_count = 10;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "RegNet_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b3 = read_floats_at(wf, 7936, b3_count, "b3");
  float *w2 = read_floats_at(wf, 8192, w2_count, "w2");
  float *b4 = read_floats_at(wf, 155648, b4_count, "b4");
  float *b5 = read_floats_at(wf, 156416, b5_count, "b5");
  float *b6 = read_floats_at(wf, 156672, b6_count, "b6");
  float *w3 = read_floats_at(wf, 156928, w3_count, "w3");
  float *b7 = read_floats_at(wf, 451840, b7_count, "b7");
  float *b8 = read_floats_at(wf, 453376, b8_count, "b8");
  float *b9 = read_floats_at(wf, 453888, b9_count, "b9");
  float *w4 = read_floats_at(wf, 454400, w4_count, "w4");
  float *b10 = read_floats_at(wf, 1044224, b10_count, "b10");
  float *b11 = read_floats_at(wf, 1045760, b11_count, "b11");
  float *b12 = read_floats_at(wf, 1046272, b12_count, "b12");
  float *w5 = read_floats_at(wf, 1046784, w5_count, "w5");
  float *b13 = read_floats_at(wf, 2226432, b13_count, "b13");
  float *b14 = read_floats_at(wf, 2229504, b14_count, "b14");
  float *b15 = read_floats_at(wf, 2230528, b15_count, "b15");
  float *w6 = read_floats_at(wf, 2231552, w6_count, "w6");
  float *b16 = read_floats_at(wf, 4590848, b16_count, "b16");
  float *b17 = read_floats_at(wf, 4593920, b17_count, "b17");
  float *b18 = read_floats_at(wf, 4594944, b18_count, "b18");
  float *w7 = read_floats_at(wf, 4595968, w7_count, "w7");
  float *b19 = read_floats_at(wf, 4606208, b19_count, "b19");
  float *b20 = read_floats_at(wf, 7168, b20_count, "b20");
  float *b21 = read_floats_at(wf, 7424, b21_count, "b21");
  float *b22 = read_floats_at(wf, 155904, b22_count, "b22");
  float *b23 = read_floats_at(wf, 156160, b23_count, "b23");
  float *b24 = read_floats_at(wf, 452352, b24_count, "b24");
  float *b25 = read_floats_at(wf, 452864, b25_count, "b25");
  float *b26 = read_floats_at(wf, 1044736, b26_count, "b26");
  float *b27 = read_floats_at(wf, 1045248, b27_count, "b27");
  float *b28 = read_floats_at(wf, 2227456, b28_count, "b28");
  float *b29 = read_floats_at(wf, 2228480, b29_count, "b29");
  float *b30 = read_floats_at(wf, 4591872, b30_count, "b30");
  float *b31 = read_floats_at(wf, 4592896, b31_count, "b31");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "RegNet", "mlir_llvm", run_id,
