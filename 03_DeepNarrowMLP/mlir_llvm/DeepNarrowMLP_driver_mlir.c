 ...
 * signature, so they were borrowed from sibling .mlir (./03_DeepNarrowMLP/mlir_llvm/DeepNarrowMLP_mlir.mlir)
 ...
 * in weights.bin is its byte offset from DeepNarrowMLP_params.json,
 * looked up by canonical torch parameter name:
 * x: shape [1024, 8192] (input)
 * w1: shape [1024, 8192] (weight) <- network.0.weight @ byte 0
 * b1: shape [1024] (bias) <- network.0.bias @ byte 33554432
 * w2: shape [1024, 1024] (weight) <- network.2.weight @ byte 33558528
 * b2: shape [1024] (bias) <- network.2.bias @ byte 37752832
 * w3: shape [1024, 1024] (weight) <- network.4.weight @ byte 37756928
 * b3: shape [1024] (bias) <- network.4.bias @ byte 41951232
 * w4: shape [1024, 1024] (weight) <- network.6.weight @ byte 41955328
 * b4: shape [1024] (bias) <- network.6.bias @ byte 46149632
 * w5: shape [1024, 1024] (weight) <- network.8.weight @ byte 46153728
 * b5: shape [1024] (bias) <- network.8.bias @ byte 50348032
 * w6: shape [1024, 1024] (weight) <- network.10.weight @ byte 50352128
 * b6: shape [1024] (bias) <- network.10.bias @ byte 54546432
 * w7: shape [1024, 1024] (weight) <- network.12.weight @ byte 54550528
 * b7: shape [1024] (bias) <- network.12.bias @ byte 58744832
 * w8: shape [1024, 1024] (weight) <- network.14.weight @ byte 58748928
 * b8: shape [1024] (bias) <- network.14.bias @ byte 62943232
 * w9: shape [1024, 1024] (weight) <- network.16.weight @ byte 62947328
 * b9: shape [1024] (bias) <- network.16.bias @ byte 67141632
 * w10: shape [1024, 1024] (weight) <- network.18.weight @ byte 67145728
 * b10: shape [1024] (bias) <- network.18.bias @ byte 71340032
 * w11: shape [1024, 1024] (weight) <- network.20.weight @ byte 71344128
 * b11: shape [1024] (bias) <- network.20.bias @ byte 75538432
 * w12: shape [1024, 1024] (weight) <- network.22.weight @ byte 75542528
 * b12: shape [1024] (bias) <- network.22.bias @ byte 79736832
 * w13: shape [1024, 1024] (weight) <- network.24.weight @ byte 79740928
 * b13: shape [1024] (bias) <- network.24.bias @ byte 83935232
 * w14: shape [1024, 1024] (weight) <- network.26.weight @ byte 83939328
 * b14: shape [1024] (bias) <- network.26.bias @ byte 88133632
 * w15: shape [1024, 1024] (weight) <- network.28.weight @ byte 88137728
 * b15: shape [1024] (bias) <- network.28.bias @ byte 92332032
 * w16: shape [1024, 1024] (weight) <- network.30.weight @ byte 92336128
 * b16: shape [1024] (bias) <- network.30.bias @ byte 96530432
 * w17: shape [8192, 1024] (weight) <- network.32.weight @ byte 96534528
 * b17: shape [8192] (bias) <- network.32.bias @ byte 130088960
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "DeepNarrowMLP_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 33554432, b1_count, "b1");
  float *w2 = read_floats_at(wf, 33558528, w2_count, "w2");
  float *b2 = read_floats_at(wf, 37752832, b2_count, "b2");
  float *w3 = read_floats_at(wf, 37756928, w3_count, "w3");
  float *b3 = read_floats_at(wf, 41951232, b3_count, "b3");
  float *w4 = read_floats_at(wf, 41955328, w4_count, "w4");
  float *b4 = read_floats_at(wf, 46149632, b4_count, "b4");
  float *w5 = read_floats_at(wf, 46153728, w5_count, "w5");
  float *b5 = read_floats_at(wf, 50348032, b5_count, "b5");
  float *w6 = read_floats_at(wf, 50352128, w6_count, "w6");
  float *b6 = read_floats_at(wf, 54546432, b6_count, "b6");
  float *w7 = read_floats_at(wf, 54550528, w7_count, "w7");
  float *b7 = read_floats_at(wf, 58744832, b7_count, "b7");
  float *w8 = read_floats_at(wf, 58748928, w8_count, "w8");
  float *b8 = read_floats_at(wf, 62943232, b8_count, "b8");
  float *w9 = read_floats_at(wf, 62947328, w9_count, "w9");
  float *b9 = read_floats_at(wf, 67141632, b9_count, "b9");
  float *w10 = read_floats_at(wf, 67145728, w10_count, "w10");
  float *b10 = read_floats_at(wf, 71340032, b10_count, "b10");
  float *w11 = read_floats_at(wf, 71344128, w11_count, "w11");
  float *b11 = read_floats_at(wf, 75538432, b11_count, "b11");
  float *w12 = read_floats_at(wf, 75542528, w12_count, "w12");
  float *b12 = read_floats_at(wf, 79736832, b12_count, "b12");
  float *w13 = read_floats_at(wf, 79740928, w13_count, "w13");
  float *b13 = read_floats_at(wf, 83935232, b13_count, "b13");
  float *w14 = read_floats_at(wf, 83939328, w14_count, "w14");
  float *b14 = read_floats_at(wf, 88133632, b14_count, "b14");
  float *w15 = read_floats_at(wf, 88137728, w15_count, "w15");
  float *b15 = read_floats_at(wf, 92332032, b15_count, "b15");
  float *w16 = read_floats_at(wf, 92336128, w16_count, "w16");
  float *b16 = read_floats_at(wf, 96530432, b16_count, "b16");
  float *w17 = read_floats_at(wf, 96534528, w17_count, "w17");
  float *b17 = read_floats_at(wf, 130088960, b17_count, "b17");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "DeepNarrowMLP", "mlir_llvm", run_id,
