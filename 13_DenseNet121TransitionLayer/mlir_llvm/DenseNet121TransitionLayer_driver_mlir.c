...
/* Auto-generated driver for `DenseNet121TransitionLayer` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./13_DenseNet121TransitionLayer/mlir_llvm/DenseNet121TransitionLayer_mlir.mlir)
 ...
 * in weights.bin is its byte offset from DenseNet121TransitionLayer_params.json,
 * looked up by canonical torch parameter name:
 * x: shape [128, 32, 256, 256] (input)
 * b1: shape [32] (bias) <- transition.0.weight @ byte 256
 * b2: shape [32] (bias) <- transition.0.bias @ byte 384
 * w1: shape [64, 32, 1, 1] (weight) <- transition.2.weight @ byte 512
 * b3: shape [32] (bias) <- transition.0.running_mean @ byte 0
 * b4: shape [32] (bias) <- transition.0.running_var @ byte 128
 * output: shape [128, 64, 128, 128]
 *
 * Usage: DenseNet121TransitionLayer_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
extern memref4d DenseNet121TransitionLayer(
    float *x_a,
    float *x_al,
    int64_t x_off,
    int64_t x_s0,
...
static const size_t x_count = 268435456;
static const size_t b1_count = 32;
static const size_t b2_count = 32;
static const size_t w1_count = 2048;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "DenseNet121TransitionLayer_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b3 = read_floats_at(wf, 0, b3_count, "b3");
  float *b4 = read_floats_at(wf, 128, b4_count, "b4");
  fclose(wf);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      memref4d out = DenseNet121TransitionLayer(
          x, x, 0,
        128, 32, 256, 256,
        2097152, 65536, 256, 1,
        b1, b1, 0,
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        memref4d cur = DenseNet121TransitionLayer(
            x, x, 0,
        128, 32, 256, 256,
        2097152, 65536, 256, 1,
        b1, b1, 0,
  ...
  write_timing_report(json_report_path, "DenseNet121TransitionLayer", "mlir_llvm", run_id,
