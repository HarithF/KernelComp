 ...
 * signature, so they were borrowed from sibling .mlir (./13_DenseNet121TransitionLayer/mim_mlir_llvm/DenseNet121TransitionLayer_mim_mlir.mlir)
 ...
 * b1: shape [32] (bias) <- transition.0.running_mean @ byte 0
 * b2: shape [32] (bias) <- transition.0.running_var @ byte 128
 * b3: shape [32] (bias) <- transition.0.weight @ byte 256
 * b4: shape [32] (bias) <- transition.0.bias @ byte 384
 * w1: shape [64, 32] (weight) <- transition.2.weight @ byte 512
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "DenseNet121TransitionLayer_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  write_timing_report(json_report_path, "DenseNet121TransitionLayer", "mim_mlir_llvm", run_id,
