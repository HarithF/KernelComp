...
/* Auto-generated driver for `DenseNet121TransitionLayer` ("mim" raw-pointer calling convention).
 ...
 *
 * Usage: DenseNet121TransitionLayer_driver_mim <weights> <input> <reps> <output> [run_id] [json_report]
 ...
 * timing reports.
 */
extern float *DenseNet121TransitionLayer(float *x, float *b1, float *b2, float *b3, float *b4, float *w1);
...
static const size_t OUT_COUNT = 134217728;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "DenseNet121TransitionLayer_mim_llvm_%s_timing.json", run_id);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      float *out = DenseNet121TransitionLayer(x, b1, b2, b3, b4, w1);
      free(out);
      _exit(0);
    }
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        float *cur = DenseNet121TransitionLayer(x, b1, b2, b3, b4, w1);
  ...
  write_timing_report(json_report_path, "DenseNet121TransitionLayer", "mim_llvm", run_id,
