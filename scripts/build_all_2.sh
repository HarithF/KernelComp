...
#
# Intermediate .o files are deleted once each variant's build attempt
# finishes (success or failure) -- only the .ll/.c sources and the final
# binary are left behind.
#
set -euo pipefail
...
for arg in "$@"; do
  ...
  *)
    if [[ -d "$arg" ]]; then
      ...
      echo "usage: $0 [MODELS_ROOT] [OPT_LEVEL] [LLVM_LIB_DIR] [-native]" >&2
      exit 1
    fi
    ;;
  esac
