 ...
 * signature, so they were borrowed from sibling .mlir (./14_DenseNet121DenseBlock/mlir_llvm/DenseNet121DenseBlock_mlir.mlir)
 ...
 * b1: shape [32] (bias) <- layers.0.0.weight @ byte 256
 * b2: shape [32] (bias) <- layers.0.0.bias @ byte 384
 * w1: shape [32, 32, 3, 3] (weight) <- layers.0.2.weight @ byte 512
 * b3: shape [64] (bias) <- layers.1.0.weight @ byte 37888
 * b4: shape [64] (bias) <- layers.1.0.bias @ byte 38144
 * w2: shape [32, 64, 3, 3] (weight) <- layers.1.2.weight @ byte 38400
 * b5: shape [96] (bias) <- layers.2.0.weight @ byte 112896
 * b6: shape [96] (bias) <- layers.2.0.bias @ byte 113280
 * w3: shape [32, 96, 3, 3] (weight) <- layers.2.2.weight @ byte 113664
 * b7: shape [128] (bias) <- layers.3.0.weight @ byte 225280
 * b8: shape [128] (bias) <- layers.3.0.bias @ byte 225792
 * w4: shape [32, 128, 3, 3] (weight) <- layers.3.2.weight @ byte 226304
 * b9: shape [160] (bias) <- layers.4.0.weight @ byte 375040
 * b10: shape [160] (bias) <- layers.4.0.bias @ byte 375680
 * w5: shape [32, 160, 3, 3] (weight) <- layers.4.2.weight @ byte 376320
 * b11: shape [192] (bias) <- layers.5.0.weight @ byte 562176
 * b12: shape [192] (bias) <- layers.5.0.bias @ byte 562944
 * w6: shape [32, 192, 3, 3] (weight) <- layers.5.2.weight @ byte 563712
 * b13: shape [32] (bias) <- layers.0.0.running_mean @ byte 0
 * b14: shape [32] (bias) <- layers.0.0.running_var @ byte 128
 * b15: shape [64] (bias) <- layers.1.0.running_mean @ byte 37376
 * b16: shape [64] (bias) <- layers.1.0.running_var @ byte 37632
 * b17: shape [96] (bias) <- layers.2.0.running_mean @ byte 112128
 * b18: shape [96] (bias) <- layers.2.0.running_var @ byte 112512
 * b19: shape [128] (bias) <- layers.3.0.running_mean @ byte 224256
 * b20: shape [128] (bias) <- layers.3.0.running_var @ byte 224768
 * b21: shape [160] (bias) <- layers.4.0.running_mean @ byte 373760
 * b22: shape [160] (bias) <- layers.4.0.running_var @ byte 374400
 * b23: shape [192] (bias) <- layers.5.0.running_mean @ byte 560640
 * b24: shape [192] (bias) <- layers.5.0.running_var @ byte 561408
...
static const size_t b6_count = 96;
...
static const size_t b21_count = 160;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "DenseNet121DenseBlock_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 256, b1_count, "b1");
  float *b2 = read_floats_at(wf, 384, b2_count, "b2");
  float *w1 = read_floats_at(wf, 512, w1_count, "w1");
  float *b3 = read_floats_at(wf, 37888, b3_count, "b3");
  float *b4 = read_floats_at(wf, 38144, b4_count, "b4");
  float *w2 = read_floats_at(wf, 38400, w2_count, "w2");
  float *b5 = read_floats_at(wf, 112896, b5_count, "b5");
  float *b6 = read_floats_at(wf, 113280, b6_count, "b6");
  float *w3 = read_floats_at(wf, 113664, w3_count, "w3");
  float *b7 = read_floats_at(wf, 225280, b7_count, "b7");
  float *b8 = read_floats_at(wf, 225792, b8_count, "b8");
  float *w4 = read_floats_at(wf, 226304, w4_count, "w4");
  float *b9 = read_floats_at(wf, 375040, b9_count, "b9");
  float *b10 = read_floats_at(wf, 375680, b10_count, "b10");
  float *w5 = read_floats_at(wf, 376320, w5_count, "w5");
  float *b11 = read_floats_at(wf, 562176, b11_count, "b11");
  float *b12 = read_floats_at(wf, 562944, b12_count, "b12");
  float *w6 = read_floats_at(wf, 563712, w6_count, "w6");
  float *b13 = read_floats_at(wf, 0, b13_count, "b13");
  float *b14 = read_floats_at(wf, 128, b14_count, "b14");
  float *b15 = read_floats_at(wf, 37376, b15_count, "b15");
  float *b16 = read_floats_at(wf, 37632, b16_count, "b16");
  float *b17 = read_floats_at(wf, 112128, b17_count, "b17");
  float *b18 = read_floats_at(wf, 112512, b18_count, "b18");
  float *b19 = read_floats_at(wf, 224256, b19_count, "b19");
  float *b20 = read_floats_at(wf, 224768, b20_count, "b20");
  float *b21 = read_floats_at(wf, 373760, b21_count, "b21");
  float *b22 = read_floats_at(wf, 374400, b22_count, "b22");
  float *b23 = read_floats_at(wf, 560640, b23_count, "b23");
  float *b24 = read_floats_at(wf, 561408, b24_count, "b24");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "DenseNet121DenseBlock", "mlir_llvm", run_id,
