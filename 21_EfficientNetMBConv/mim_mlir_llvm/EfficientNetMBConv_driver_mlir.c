...
/* Auto-generated driver for `EfficientNetMBConv` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./21_EfficientNetMBConv/mim_mlir_llvm/EfficientNetMBConv_mim_mlir.mlir)
 ...
 * in weights.bin is its byte offset from EfficientNetMBConv_params.json,
 * looked up by canonical torch parameter name:
 * w1: shape [672, 112] (weight) <- expand_conv.0.weight @ byte 0
 * x: shape [10, 112, 224, 224] (input)
 * b1: shape [672] (bias) <- expand_conv.1.running_mean @ byte 301056
 * b2: shape [672] (bias) <- expand_conv.1.running_var @ byte 303744
 * b3: shape [672] (bias) <- expand_conv.1.weight @ byte 306432
 * b4: shape [672] (bias) <- expand_conv.1.bias @ byte 309120
 * w2: shape [672, 5, 5] (weight) <- depthwise_conv.0.weight @ byte 311808
 * b5: shape [672] (bias) <- depthwise_conv.1.running_mean @ byte 379008
 * b6: shape [672] (bias) <- depthwise_conv.1.running_var @ byte 381696
 * b7: shape [672] (bias) <- depthwise_conv.1.weight @ byte 384384
 * b8: shape [672] (bias) <- depthwise_conv.1.bias @ byte 387072
 * w3: shape [192, 672] (weight) <- project_conv.0.weight @ byte 389760
 * b9: shape [192] (bias) <- project_conv.1.running_mean @ byte 905856
 * b10: shape [192] (bias) <- project_conv.1.running_var @ byte 906624
 * b11: shape [192] (bias) <- project_conv.1.weight @ byte 907392
 * b12: shape [192] (bias) <- project_conv.1.bias @ byte 908160
 * output: shape [10, 192, 112, 112]
 *
 * Usage: EfficientNetMBConv_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
extern memref4d EfficientNetMBConv(
    float *w1_a,
    float *w1_al,
    int64_t w1_off,
    int64_t w1_s0,
...
static const size_t w1_count = 75264;
static const size_t x_count = 56197120;
static const size_t b1_count = 672;
static const size_t b2_count = 672;
static const size_t b3_count = 672;
static const size_t b4_count = 672;
static const size_t w2_count = 16800;
static const size_t b5_count = 672;
static const size_t b6_count = 672;
static const size_t b7_count = 672;
static const size_t b8_count = 672;
static const size_t w3_count = 129024;
static const size_t b9_count = 192;
static const size_t b10_count = 192;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "EfficientNetMBConv_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 301056, b1_count, "b1");
  float *b2 = read_floats_at(wf, 303744, b2_count, "b2");
  float *b3 = read_floats_at(wf, 306432, b3_count, "b3");
  float *b4 = read_floats_at(wf, 309120, b4_count, "b4");
  float *w2 = read_floats_at(wf, 311808, w2_count, "w2");
  float *b5 = read_floats_at(wf, 379008, b5_count, "b5");
  float *b6 = read_floats_at(wf, 381696, b6_count, "b6");
  float *b7 = read_floats_at(wf, 384384, b7_count, "b7");
  float *b8 = read_floats_at(wf, 387072, b8_count, "b8");
  float *w3 = read_floats_at(wf, 389760, w3_count, "w3");
  float *b9 = read_floats_at(wf, 905856, b9_count, "b9");
  float *b10 = read_floats_at(wf, 906624, b10_count, "b10");
  float *b11 = read_floats_at(wf, 907392, b11_count, "b11");
  float *b12 = read_floats_at(wf, 908160, b12_count, "b12");
  fclose(wf);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      memref4d out = EfficientNetMBConv(
          w1, w1, 0,
        672, 112,
        112, 1,
        x, x, 0,
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        memref4d cur = EfficientNetMBConv(
            w1, w1, 0,
        672, 112,
        112, 1,
        x, x, 0,
  ...
  write_timing_report(json_report_path, "EfficientNetMBConv", "mim_mlir_llvm", run_id,
