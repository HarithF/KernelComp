...
/* Auto-generated driver for `LeNet5` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./04_LeNet5/mlir_llvm/LeNet5_mlir.mlir)
 ...
 * x: shape [4096, 1, 32, 32] (input)
 * w1: shape [6, 1, 5, 5] (weight) <- conv1.weight @ byte 0
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "LeNet5_mlir_llvm_%s_timing.json", run_id);
  ...
  write_timing_report(json_report_path, "LeNet5", "mlir_llvm", run_id,
