...
/* Auto-generated driver for `NetVladWithGhostClusters` ("mim" raw-pointer calling convention).
 ...
 *
 * Usage: NetVladWithGhostClusters_driver_mim <weights> <input> <reps> <output> [run_id] [json_report]
 ...
 * timing reports.
 */
extern float *NetVladWithGhostClusters(float *x, float *w1, float *b1, float *b2, float *b3, float *b4, float *w2);
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "NetVladWithGhostClusters_mim_llvm_%s_timing.json", run_id);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      float *out = NetVladWithGhostClusters(x, w1, b1, b2, b3, b4, w2);
      free(out);
      _exit(0);
    }
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        float *cur = NetVladWithGhostClusters(x, w1, b1, b2, b3, b4, w2);
  ...
  write_timing_report(json_report_path, "NetVladWithGhostClusters", "mim_llvm", run_id,
