 ...
 * signature, so they were borrowed from sibling .mlir (./46_NetVladWithGhostClusters/mlir_llvm/NetVladWithGhostClusters_mlir.mlir)
 ...
 * w2: shape [1, 512, 32] (weight) <- clusters2 @ byte 99072
 * b1: shape [48] (bias) <- batch_norm.weight @ byte 98688
 * b2: shape [48] (bias) <- batch_norm.bias @ byte 98880
 * b3: shape [48] (bias) <- batch_norm.running_mean @ byte 98304
 * b4: shape [48] (bias) <- batch_norm.running_var @ byte 98496
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "NetVladWithGhostClusters_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 98688, b1_count, "b1");
  float *b2 = read_floats_at(wf, 98880, b2_count, "b2");
  float *b3 = read_floats_at(wf, 98304, b3_count, "b3");
  float *b4 = read_floats_at(wf, 98496, b4_count, "b4");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "NetVladWithGhostClusters", "mlir_llvm", run_id,
