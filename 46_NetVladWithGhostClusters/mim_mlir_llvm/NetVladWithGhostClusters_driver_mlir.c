...
/* Auto-generated driver for `NetVladWithGhostClusters` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./46_NetVladWithGhostClusters/mim_mlir_llvm/NetVladWithGhostClusters_mim_mlir.mlir)
 ...
 * in weights.bin is its byte offset from NetVladWithGhostClusters_params.json,
 * looked up by canonical torch parameter name:
 * x: shape [2048, 100, 512] (input)
 * w1: shape [512, 48] (weight) <- clusters @ byte 0
 * b1: shape [48] (bias) <- batch_norm.running_mean @ byte 98304
 * b2: shape [48] (bias) <- batch_norm.running_var @ byte 98496
 * b3: shape [48] (bias) <- batch_norm.weight @ byte 98688
 * b4: shape [48] (bias) <- batch_norm.bias @ byte 98880
 * w2: shape [512, 32] (weight) <- clusters2 @ byte 99072
 * output: shape [2048, 16384]
 *
 * Usage: NetVladWithGhostClusters_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
extern memref2d NetVladWithGhostClusters(
    float *x_a,
    float *x_al,
    int64_t x_off,
    int64_t x_s0,
...
static const size_t w1_count = 24576;
static const size_t b1_count = 48;
static const size_t b2_count = 48;
static const size_t b3_count = 48;
static const size_t b4_count = 48;
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "NetVladWithGhostClusters_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 98304, b1_count, "b1");
  float *b2 = read_floats_at(wf, 98496, b2_count, "b2");
  float *b3 = read_floats_at(wf, 98688, b3_count, "b3");
  float *b4 = read_floats_at(wf, 98880, b4_count, "b4");
  float *w2 = read_floats_at(wf, 99072, w2_count, "w2");
  fclose(wf);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      memref2d out = NetVladWithGhostClusters(
          x, x, 0,
        2048, 100, 512,
        51200, 512, 1,
        w1, w1, 0,
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        memref2d cur = NetVladWithGhostClusters(
            x, x, 0,
        2048, 100, 512,
        51200, 512, 1,
        w1, w1, 0,
  ...
  write_timing_report(json_report_path, "NetVladWithGhostClusters", "mim_mlir_llvm", run_id,
