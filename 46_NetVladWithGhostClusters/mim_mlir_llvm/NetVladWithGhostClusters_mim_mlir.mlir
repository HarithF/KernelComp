module {
  func.func public @NetVladWithGhostClusters(%l_x_: tensor<2048x100x512xf32>, %l_self_parameters_clusters_: tensor<512x48xf32>, %l_self_modules_batch_norm_buffers_running_mean_: tensor<48xf32>, %l_self_modules_batch_norm_buffers_running_var_: tensor<48xf32>, %l_self_modules_batch_norm_parameters_weight_: tensor<48xf32>, %l_self_modules_batch_norm_parameters_bias_: tensor<48xf32>, %l_self_parameters_clusters2_: tensor<512x32xf32>) -> tensor<2048x16384xf32> {
    %assignment_1.empty = tensor.empty() : tensor<204800x48xf32>
    %init = arith.constant 0.0 : f32
    %assignment_1.buf = linalg.fill ins(%init : f32) outs(%assignment_1.empty : tensor<204800x48xf32>) -> tensor<204800x48xf32>
    ...
    %assignment_1.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0 floordiv 100, d0 mod 100, d2 * 4 + d3)>, affine_map<(d0, d1, d2, d3) -> (d2 * 4 + d3, d1)>, affine_map<(d0, d1, d2, d3) -> (d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction", "reduction"]}
      ins(%l_x_, %l_self_parameters_clusters_, %assignment_1.shape : tensor<2048x100x512xf32>, tensor<512x48xf32>, tensor<128x4xf32>)
      outs(%assignment_1.buf : tensor<204800x48xf32>) {
    ...
    %v6.buf = tensor.empty() : tensor<48xf32>
    %v6.buf.out = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]}
      ins(%l_self_modules_batch_norm_buffers_running_var_, %l_self_modules_batch_norm_parameters_weight_ : tensor<48xf32>, tensor<48xf32>)
      outs(%v6.buf : tensor<48xf32>) {
    ...
    -> tensor<48xf32>
    %v16.buf = tensor.empty() : tensor<48xf32>
    %v16.buf.out = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]}
      ins(%l_self_modules_batch_norm_buffers_running_mean_, %l_self_modules_batch_norm_buffers_running_var_, %l_self_modules_batch_norm_parameters_weight_ : tensor<48xf32>, tensor<48xf32>, tensor<48xf32>)
      outs(%v16.buf : tensor<48xf32>) {
    ...
    -> tensor<48xf32>
    %assignment_1.post = tensor.empty() : tensor<204800x48xf32>
    %assignment_1.post.out = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]}
      ins(%assignment_1.buf.out, %v6.buf.out, %v16.buf.out, %l_self_modules_batch_norm_parameters_bias_ : tensor<204800x48xf32>, tensor<48xf32>, tensor<48xf32>, tensor<48xf32>)
      outs(%assignment_1.post : tensor<204800x48xf32>) {
    ...
    -> tensor<204800x48xf32>
    %assignment_1.post.out.slice = tensor.extract_slice %assignment_1.post.out[0, 0] [204800, 32] [1, 1] : tensor<204800x48xf32> to tensor<204800x32xf32>
    ...
    %input.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]}
      ins(%assignment_1.post.out : tensor<204800x48xf32>)
    ...
    %input_1.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0)>], iterator_types = ["parallel", "reduction"]}
      ins(%assignment_1.post.out, %input.buf.out : tensor<204800x48xf32>, tensor<204800xf32>)
    ...
    -> tensor<204800xf32>
    %assignment_3.buf = tensor.empty() : tensor<204800x32xf32>
    %assignment_3.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]}
      ins(%assignment_1.post.out.slice, %input.buf.out, %input_1.buf.out : tensor<204800x32xf32>, tensor<204800xf32>, tensor<204800xf32>)
      outs(%assignment_3.buf : tensor<204800x32xf32>) {
    ...
    %assignment_5.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d2 + d0 * 100, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]}
      ins(%assignment_3.buf.out : tensor<204800x32xf32>)
    ...
    %a_sum.buf.out = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d3 + d0 * 100, d2)>, affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "reduction"]}
      ins(%assignment_3.buf.out, %a_sum.shape : tensor<204800x32xf32>, tensor<100xf32>)
