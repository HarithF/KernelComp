 ...
 * signature, so they were borrowed from sibling .mlir (./17_SqueezeNetFireModule/mlir_llvm/SqueezeNetFireModule_mlir.mlir)
 ...
 * w1: shape [6, 3, 1, 1] (weight) <- squeeze.weight @ byte 0
 * b1: shape [6] (bias) <- squeeze.bias @ byte 72
 * w2: shape [64, 6, 1, 1] (weight) <- expand1x1.weight @ byte 96
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "SqueezeNetFireModule_mlir_llvm_%s_timing.json", run_id);
  ...
  write_timing_report(json_report_path, "SqueezeNetFireModule", "mlir_llvm", run_id,
