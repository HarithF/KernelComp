...
/* Auto-generated driver for `AlexNet` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./05_AlexNet/mim_mlir_llvm/AlexNet_mim_mlir.mlir)
 ...
 * in weights.bin is its byte offset from AlexNet_params.json,
 * looked up by canonical torch parameter name:
 * w1: shape [96, 3, 11, 11] (weight) <- conv1.weight @ byte 0
 * b1: shape [96] (bias) <- conv1.bias @ byte 139392
 * x: shape [1024, 3, 224, 224] (input)
 * w2: shape [256, 96, 5, 5] (weight) <- conv2.weight @ byte 139776
 * b2: shape [256] (bias) <- conv2.bias @ byte 2597376
 * w3: shape [384, 256, 3, 3] (weight) <- conv3.weight @ byte 2598400
 * b3: shape [384] (bias) <- conv3.bias @ byte 6137344
 * w4: shape [384, 384, 3, 3] (weight) <- conv4.weight @ byte 6138880
 * b4: shape [384] (bias) <- conv4.bias @ byte 11447296
 * w5: shape [256, 384, 3, 3] (weight) <- conv5.weight @ byte 11448832
 * b5: shape [256] (bias) <- conv5.bias @ byte 14987776
 * w6: shape [4096, 9216] (weight) <- fc1.weight @ byte 14988800
 * b6: shape [4096] (bias) <- fc1.bias @ byte 165983744
 * w7: shape [4096, 4096] (weight) <- fc2.weight @ byte 166000128
 * b7: shape [4096] (bias) <- fc2.bias @ byte 233108992
 * w8: shape [1000, 4096] (weight) <- fc3.weight @ byte 233125376
 * b8: shape [1000] (bias) <- fc3.bias @ byte 249509376
 * output: shape [1024, 1000]
 *
 * Usage: AlexNet_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "AlexNet_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 139392, b1_count, "b1");
  float *w2 = read_floats_at(wf, 139776, w2_count, "w2");
  float *b2 = read_floats_at(wf, 2597376, b2_count, "b2");
  float *w3 = read_floats_at(wf, 2598400, w3_count, "w3");
  float *b3 = read_floats_at(wf, 6137344, b3_count, "b3");
  float *w4 = read_floats_at(wf, 6138880, w4_count, "w4");
  float *b4 = read_floats_at(wf, 11447296, b4_count, "b4");
  float *w5 = read_floats_at(wf, 11448832, w5_count, "w5");
  float *b5 = read_floats_at(wf, 14987776, b5_count, "b5");
  float *w6 = read_floats_at(wf, 14988800, w6_count, "w6");
  float *b6 = read_floats_at(wf, 165983744, b6_count, "b6");
  float *w7 = read_floats_at(wf, 166000128, w7_count, "w7");
  float *b7 = read_floats_at(wf, 233108992, b7_count, "b7");
  float *w8 = read_floats_at(wf, 233125376, w8_count, "w8");
  float *b8 = read_floats_at(wf, 249509376, b8_count, "b8");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "AlexNet", "mim_mlir_llvm", run_id,
