 ...
 * signature, so they were borrowed from sibling .mlir (./47_NetVladNoGhostClusters/mlir_llvm/NetVladNoGhostClusters_mlir.mlir)
 ...
 * w2: shape [1, 512, 32] (weight) <- clusters2 @ byte 66048
 * b1: shape [32] (bias) <- batch_norm.weight @ byte 65792
 * b2: shape [32] (bias) <- batch_norm.bias @ byte 65920
 * b3: shape [32] (bias) <- batch_norm.running_mean @ byte 65536
 * b4: shape [32] (bias) <- batch_norm.running_var @ byte 65664
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "NetVladNoGhostClusters_mlir_llvm_%s_timing.json", run_id);
  ...
  float *b1 = read_floats_at(wf, 65792, b1_count, "b1");
  float *b2 = read_floats_at(wf, 65920, b2_count, "b2");
  float *b3 = read_floats_at(wf, 65536, b3_count, "b3");
  float *b4 = read_floats_at(wf, 65664, b4_count, "b4");
  fclose(wf);
  ...
  write_timing_report(json_report_path, "NetVladNoGhostClusters", "mlir_llvm", run_id,
