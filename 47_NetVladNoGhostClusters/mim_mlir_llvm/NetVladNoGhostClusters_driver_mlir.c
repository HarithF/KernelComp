...
/* Auto-generated driver for `NetVladNoGhostClusters` matching MLIR's expanded memref
 ...
 * signature, so they were borrowed from sibling .mlir (./47_NetVladNoGhostClusters/mim_mlir_llvm/NetVladNoGhostClusters_mim_mlir.mlir)
 ...
 * in weights.bin is its byte offset from NetVladNoGhostClusters_params.json,
 ...
 *
 * Usage: NetVladNoGhostClusters_driver_mlir <weights> <input> <reps> <output> [run_id] [json_report]
...
extern memref2d NetVladNoGhostClusters(
    float *x_a,
    float *x_al,
    int64_t x_off,
    int64_t x_s0,
...
int main(int argc, char **argv) {
  ...
  if (argc > 6) {
    ...
    snprintf(json_path_buf, sizeof(json_path_buf), "NetVladNoGhostClusters_mim_mlir_llvm_%s_timing.json", run_id);
  ...
  if (use_fork) {
    ...
    if (p == 0) {
      memref2d out = NetVladNoGhostClusters(
          x, x, 0,
        2048, 100, 512,
        51200, 512, 1,
        w1, w1, 0,
  ...
  for (int i = 0; i < reps; i++) {
    ...
    if (use_fork) {
      ...
      if (p == 0) {
        ...
        memref2d cur = NetVladNoGhostClusters(
            x, x, 0,
        2048, 100, 512,
        51200, 512, 1,
        w1, w1, 0,
  ...
  write_timing_report(json_report_path, "NetVladNoGhostClusters", "mim_mlir_llvm", run_id,
